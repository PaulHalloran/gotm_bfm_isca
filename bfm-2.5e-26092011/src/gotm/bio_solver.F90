...
#ifdef BFM_GOTM
   if (bio_setup/=2) then
     cc = (cc+dt*sum(pp(1:numc,1:numc,:),2))/(1.+dt*sum(dd(1:numc,1:numc,:),2)/(1.0D-80+cc))
   end if
   ! compute benthic variables 
    if (bio_setup>1) then
      ccb = (ccb+dt*sum(ppb(:,:,:),2))/(1.+dt*sum(ddb(:,:,:),2)/(1.0D-80+ccb))
    end if
#else
   do ci=1,nlev
      do i=1,numc
     ...
     do ci=1,NLEVB
        do i=1,numbc
           ...
           do j=1,numbc
              ...
              if (i.ne.j) a(i,j)=-dt*ppb(i,j,ci)/(1.0D-80+ccb(j,ci))
           end do
           a(i,i)=dt*a(i,i)/(1.0D-80+ccb(i,ci))
           a(i,i)=_ONE_+a(i,i)
