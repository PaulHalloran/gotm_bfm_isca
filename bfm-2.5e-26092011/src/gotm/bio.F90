...
! !USES:
   ...
   use bio_bfm, only : reset_diagonal, allocate_memory_bfm,test_on_all_negative_states,  &
                       test_on_negative_states,test_mass_conservation !, do_bfm_river_loads

   use trace_bdy, only:init_trace_bdy,init_var_trace

#endif

   use output, only: out_fmt,write_results
...
! !PUBLIC MEMBER FUNCTIONS:
   public init_bio, do_bio, end_bio,d3_administration
...
!
! !INTERFACE:
   subroutine init_bio(namlst,fname,unit,nlev,s,t,h)
!
! !DESCRIPTION:
...
!     a sanity check (only temporarely)
      ...
      case (6)  ! The BFM (ERSEM) model
         ...
         call init_var_bfm(namlst,'bio_bfm.inp',unit,bio_setup,nlev,s,t,h)

       case (7)  ! The trace model

         call init_trace_bdy('bio_trace.inp',nlev)
...
! !INTERFACE:
#ifdef BFM_GOTM
   subroutine do_bio(mode,nlev,I_0,dl,uwind,vwind,u_taub,dt,h,t,s,rho,nuh,rad,bioshade,counter)
#else
...
#ifdef BFM_GOTM
   use mem, only: track_error ! this variable is used to control debug output in BFM
   use gotm_error_msg, only:gotm_error,set_warning_for_getm , &
                            get_d3_model_flag_from_getm
!  use mem,only:ppN1p,source_d3_vector
#endif
   IMPLICIT NONE
!
! !INPUT PARAMETERS:
   ...
   REALTYPE, intent(in)                :: uwind,vwind,u_taub
...
!BOC
   if (bio_calc) then
      ...
      llsumh=(sum(h(1:nlev)) .gt. 0.21D+00) 
        if (bio_model==6.and.nega_test_after_horadv) then
          call test_on_all_negative_states (d3_model_flag, llsumh,bio_setup, h, numcc,nlev, "hor_adv", cc, kt )
          if ( kt.lt.0) then
...
!        Very important for 3D models to save extra 3D field:
...
#ifdef BFM_GOTM
            case (6)
               call envforcing_bfm(nlev,h,t,s,rho,I_0,dl,uwind,vwind,u_taub,bioshade_feedback,bioshade)
#endif
           end select

#ifdef BFM_GOTM
           call test_mass_conservation(.TRUE.,1,'',totsysn_old,totsysp_old)
#endif

           if ( bio_setup >0 ) & 
                  call ode_solver(ode_method,numc,nlev,dt_eff,h,cc,t)
#ifdef BFM_GOTM
           call test_on_all_negative_states (nega_test_after_odv, llsumh,bio_setup, h, numcc,nlev, "ode_solver", cc, kt )
           if ( kt.lt.0) then
           endif
           if ( split_factor > 1) then
             call test_mass_conservation(mass_test_after_odv,4,'ode_solver',totsysn_old,totsysp_old)
          else
             call test_mass_conservation(.TRUE.,2,'',totsysn_old,totsysp_old)
          endif
#endif
      ...
      if (bio_eulerian) then
        ...
        if (bio_model==6) then
          ...
          do j=1,numcc
            if (bio_setup /= 2 ) then
              if (pelvar_type(j)>=ALLTRANSPORT) then
                ...
                c1dimz=cc(j,:)
                if (llsumh ) then
                    call test_on_negative_states ( j,llsumh,h,nlev, "bio+advection+vdiff",c1dimz, kt,i )
                    if ( llws(j)) call adv_center_bfm(j,nlev,dt,h,h,ws(j,:),flux, &
                                       flux,_ZERO_,_ZERO_,w_adv_discr,mass_conservation_adv,c1dimz)
                ! do diffusion step
                    call diff_center_bfm(nlev,dt,cnpar,posconc(j),h,Neumann,Neumann,&
                       sfl(j),bfl(j),nuh,Lsour,Qsour,RelaxTau,c1dimz,mass_conservation_diff,c1dimz)
                    call test_on_negative_states ( j,llsumh,h,nlev, "bio+advection+vdiff",c1dimz, kt,i )
                else
                    call diff_center_bfm(nlev,dt,cnpar,posconc(j),h,Neumann,Neumann,&
                       _ZERO_,_ZERO_,nuh,Lsour,Qsour,RelaxTau,c1dimz,mass_conservation_diff,c1dimz)
                     call test_on_negative_states ( j,llsumh,h,nlev, "bio+vert.diff" ,c1dimz,kt)
                endif
                cc(j,:)=c1dimz
          ...
          enddo
          if (present(counter)) counter=i

        
#ifdef BFM_GOTM
!         call test_mass_conservation(llsumh,3,'advection/diffusion',totsysn_old,totsysp_old)
#endif
          if ( kt.lt.0) then
...
!           do diffusion step
            ...
            call diff_center(nlev,dt,cnpar,posconc(j),h,Neumann,Neumann,&
                sfl(j),bfl(j),nuh,Lsour,Qsour,RelaxTau,c1dimz,c1dimz)
            call test_on_negative_states ( j,llsumh,h,nlev, "bio+vert.diff" ,c1dimz,kt )
            cc(j,:)=c1dimz
          end do
          ...
          do j=1,numcc
!           do advection step
            if ( bio_model /= 7 ) call adv_center(nlev,dt,h,h,ws(j,:),flux, &
