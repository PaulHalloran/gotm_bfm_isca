...
!
! !INTERFACE:
   subroutine adv_center_bfm(statenr,N,dt,h,ho,ww,Bcup,Bcdw,Yup,Ydw,method,mode,Y)
!
! !DESCRIPTION:
!
...
!     write(msg, &
!      '(''Warning adv_center:Max Courant N.='',G12.4,'' iterations='',I4)') &
!      cmax,it
         ...
         select case (method)
            ...
            case ((P2),(P2_PDM))
               if (method.eq.P2) then
                  ...
                  limit=max(_ZERO_,min(Phi,2.0D+00/(1.0D+00-c),2.0D+00*r/(c+1.D-10)))
               end if
            case (Superbee)
...
!     vertical flow velocity and for settling of suspended matter.
      ...
      if (mode.eq.0) then ! non-conservative 
         do k=1,N
            Y(k)=Y(k)-1.0D+00/float(it)*dt*((cu(k)-cu(k-1))/        &
                 h(k)-Y(k)*(ww(k)-ww(k-1))/h(k))
         enddo
      else                ! conservative  
         do k=1,N
            Y(k)=Y(k)-1.0D+00/float(it)*dt*((cu(k)-cu(k-1))/h(k))
         enddo
      end if   
   ...
   if ( mode.ge.2  ) then
      ...
      if ( mode.eq.3) then
        ...
        if ( s1 > 1.0D-6 ) write(*,*) ' No balance in adv_center diff=',s1*100,'%'
      endif
      Y(1:n)=Y(1:n)*(s/s2)
   endif
