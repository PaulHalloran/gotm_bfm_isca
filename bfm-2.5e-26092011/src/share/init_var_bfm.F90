...
!
! !INTERFACE:
   subroutine init_var_bfm(namlst,fname,unit,setup,nlev,s,t,h)
!
! !DESCRIPTION:
...
#endif
   use mem
   use mem_Phyto, ONLY: p_qnRc,p_qpRc,p_qsRc,p_qchlc
...
!
! !LOCAL VARIABLES:
   integer              :: icontrol,i,j,k,iiLastElement
   ...
   REALTYPE  :: N1p0,N3n0,N4n0,N5s0,N6r0,  &
                ...
                Z3c0,Z4c0,Z5c0,Z6c0,B1c0,R1c0,  &
                R2c0,R6c0,RZc0,R7c0,O2o0,O3c0,O3h0,O4n0,  &
   ...
   REALTYPE  :: Y1c0, Y2c0, Y3c0, Y4c0, Y5c0, &
                Q1c0, q11c0, Q6c0,       &
                K3n0, G4n0, K15s0, &
                H1c0, H2c0, H3c0, K1p0, K11p0, K21p0,     &
...
98 if ( icontrol == 0 ) then
   ...
   if (.NOT.empty(var_save(NSAVE))) then
      ...
      LEVEL3 "Warning:Change parameter NSAVE in init_var_bfm or limit number of var_save variables"
...
!     p_poro=p_poro0
      ...
      call init_cnps(c=H1c,n=H1n,p=H1p,nc=p_qnHc(iiH1), &
           pc=p_qpHc(iiH1))
      call init_cnps(c=H2c,n=H2n,p=H2p,nc=p_qnHc(iiH2), &
           pc=p_qpHc(iiH2))
      call init_cnps(c=H3c,n=H3n,p=H3p,nc=p_qnHc(iiH3), &
           pc=p_qpHc(iiH3))
!    end if
...
! if element of H < 0.0: a call form getm!
        ...
        STDERR 'h(1) calc_init_bennut_states=',h(1),calc_init_bennut_states
        if ( h(1) > 0.0 .and. calc_init_bennut_states >= 1) then
