...
!
  ...
  real(RLEN)  ::p_pAn2Ni=0.125           ! part of the  anoxic layer which is added to the nitrification layer to avoid (too) large
                                         ! fluxes at boundary between anoxic and denitrification layer when there is a large difference in 
  ...
  integer  :: p_N                        ! Number of reinitialization of BenthicNutrient Model
                                         ! InitCondition=1: Assume at start loss processes == input ptocesses
                                         ! InitCondition=2: Assume 0 flux at the water column at t=0
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /BenthicNutrient3_parameters/ bennut_messages,p_max_state_change,p_pAn2Ni, &
                         p_InitCondition,p_N
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
