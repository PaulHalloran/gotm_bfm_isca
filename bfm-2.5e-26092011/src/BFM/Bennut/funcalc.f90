        ...
        case (BESSELI_EXP_TERM)
          ...
          select case (mode)
            case (EXPONENTIAL_INTEGRAL)
              r=0.0D+00
              if (rx.ne.0.0D+00)r=  &
                 qgaus_exp(0.0D+00,rx,coeff%labda(1),coeff%labda(2),2.0D+00*coeff%labda(1),bessi0)
              funcalc=r;
              !equation: 11.3.25 :
!             if (r == 0.0D+00) then
!               funcalc=0.0D+00
...
!             endif
            case (INTEGRAL)
              r=0.0D+00
              if (rx.ne.0.0D+00)r=  & 
                QGAUS_EXP(0.0D+00,rx,coeff%labda(1),coeff%labda(2),0.0D+00,bessi0)
              funcalc=r
            case (EQUATION)
                funcalc=bessi0(s) 
            case (DERIVATIVE)
        ...
        case ( BESSELK_EXP_TERM)
          ...
          select case (mode)
            case (EXPONENTIAL_INTEGRAL)
              r=0.0D+00
              if (rx.ne.0.0D+00)r=  &
                 qgaus_exp(0.0D+00,rx,coeff%labda(1),coeff%labda(2),2.0D+00*coeff%labda(1),bessk0)
              funcalc=r;
!             if (r == 0.0D+00) then
!               funcalc=0.0D+00
!             else
!               funcalc=1.0D+00- s*bessk1(s)/ &
!                      (coeff%labda(1)*coeff%labda(2)*coeff%labda(2))
!             endif
            case (INTEGRAL)
              r=0.0D+00
              if (rx.ne.0.0D+00)r=  &
                 qgaus_exp(0.0D+00,rx,coeff%labda(1),coeff%labda(2),0.0D+00,bessk0)
              funcalc=r
            case (EQUATION)
              funcalc=bessk0(s) 
            case (DERIVATIVE)
