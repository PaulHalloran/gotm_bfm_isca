...
!
! !INTERFACE
  subroutine InitBenthicNutrient3Dynamics(mode)
!
! !USES:
  ...
  use mem,  ONLY:KCO2,G3c,G13c,G23c,Khplus,G3h,G13h,G23h,O3h_Ben
#endif
...
!
  ...
  ! Initial caluclation only to gete a reaonable value for D1m and D2m
    ...
    alpha=1.0/(p_small+D6m(BoxNumberXY));
...
#ifdef INCLUDE_BENCO2
  if ( mode.ge.1) then
      if (mode.eq.2) call BenthicSystemDynamics
