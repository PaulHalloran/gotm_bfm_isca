...
! !USES:
  ...
  use mem,  ONLY: K4n, G2o, D1m, K14n, D2m, K24n, D7m, D2STATE
  use mem, ONLY: ppK4n, ppK3n, ppG2o, &
    dummy,    NO_BOXES_XY, &
       BoxNumberXY, InitializeModel, LocalDelta,max_change_per_step, &
    M4n, KNH4, jG2K3o, jbotN4n, M14n, M24n, reBTn, reATn, irrenh, ETW_Ben, &
    jK4K3n, N4n_Ben, Depth_Ben, iiBen,iiH3, flux,sK4K3,source
  use constants, ONLY: LAYERS, LAYER1, LAYER2, &
    ...
    INTEGRAL, RFLUX, DERIVATIVE
  use mem_Param,  ONLY: p_poro, p_d_tot_2,p_clDxm, p_q10diff, p_qon_nitri
  use mem_BenthicNutrient3, ONLY: p_InitCondition
  use mem_Param, ONLY: CalcBenBacteria
...
!       zuBT=sign(abs(zuBT)+1.079D-6,zuBT)
        ...
        zuD1 = reATn(BoxNumberXY)/ p_poro(BoxNumberXY)/ IntegralExp( &
      ...
      if ( p_sK4K3 >ZERO .or.  (.not.CalcBenBacteria(iiH3)) ) then
      ...
      labda  =   sqrt(  max(p_slK4K3,sK4K3(BoxNumberXY))/ diff)

      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      ! Calculate coefficient of the zero order term
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

      a15  =   zuBT/ max(p_slK4K3,sK4K3(BoxNumberXY))

          if ( D1m(BoxNumberXY)< 0.0008) then
             write(LOGUNIT,*) 'D1m,sK4K3:',D1m(BoxNumberXY),sK4K3(BoxNumberXY)
      ...
      select case ( InitializeModel)
        case ( 0 )
          ...
          Tau  =   CalculateTau(  sK4K3(BoxNumberXY),  diff,  p_p,  D1m(BoxNumberXY))
          ...
          jK4K3n(BoxNumberXY)= sK4K3(BoxNumberXY)* CalculateFromSet( KNH4(BoxNumberXY), INTEGRAL, &
            RFLUX, ZERO, D1m(BoxNumberXY))

          if ( p_sK4K3 >ZERO .or.  (.not.CalcBenBacteria(3)) ) &
              call LimitChange(1,jK4K3n(BoxNumberXY),K4n(BoxNumberXY),max_change_per_step)
          ...
          r=max(ZERO,K4n(BoxNumberXY)+source(iiBen,BoxNumberXY,ppK4n)*LocalDelta)
          call LimitShift(jK4N4n,N4n_Ben(BoxNumberXY)*Depth_Ben(BoxNumberXY) ,r,max_change_per_step)

          call flux(BoxNumberXY, iiBen, ppK4n, ppK4n, -jK4N4n)

          jbotN4n(BoxNumberXY)=jK4N4n
