...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        ...
        r= p_small+max(ZERO,insw(jK10K0x)*max(p_small,K10x )+insw(-jK10K0x)* max(p_small,K0x))
        if (present(p)) then
...
!
        ...
        REAL(RLEN)                ::r
        LOGICAL                   ::ll
        ll=( mode ==1 .or. ( mode==2.and.jK0x <ZERO).or.(mode==3.and.jK0x> ZERO)) 
        r=1.0
        if ( K0x <=0.0) then
            if (ll) r=0.0;
        elseif (ll ) then
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
        ...
        N0x  =max(p_small,K0x/eK0m)
        N10x =max(p_small,K10x/eK10m)
