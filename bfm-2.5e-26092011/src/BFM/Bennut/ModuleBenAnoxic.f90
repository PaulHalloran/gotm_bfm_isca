...
!BOC
  ...
  real(RLEN)  :: p_clO2o ! half saturation constant for O2 (15 mmol O2/m3)) for reoxidation 
                         ! process of S2--
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  namelist /BenAnoxic_parameters/ p_diff, p_p, p_q10, p_sOS,p_clO2o,p_flux_at_deep_end,sw_set
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
