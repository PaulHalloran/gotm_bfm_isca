...
! !USES:
  ...
  use mem, ONLY: ppQ11c, ppQ1c, ppQ11n, ppQ1n, ppQ11p, ppQ1p, &
    dummy,    NO_BOXES_XY, &
       BoxNumberXY, LocalDelta,max_change_per_step, KQ1, irrenh, &
    ETW_Ben, shiftD1m, ruHI, reHI, iiH1, iiH2, iiBen, flux
  use constants, ONLY: LAYERS, LAYER1, LAYER2, &
    ...
    STANDARD, SET_LAYER_INTEGRAL_UNTIL, ADD, &
    SHIFT, RFLUX
  use mem_Param,  ONLY: p_poro, p_q10diff, p_clDxm, p_d_tot,p_small
  use mem_BenQ1Transport
  use mem_BenthicNutrient3, ONLY:max_change_per_step
...
!
  ...
  do BoxNumberXY=1,NO_BOXES_XY
      ...
      sQ1 = max( p_slQ1, ruHI(iiH1, BoxNumberXY)/( p_small+ Q1c(BoxNumberXY)))
      ...
      KQ1(BoxNumberXY) = InitializeSet( KQ1(BoxNumberXY), 3, 8)

      call DefineSet( KQ1(BoxNumberXY), LAYERS, LAYER1, &
                                 LAYER2, D1m(BoxNumberXY), D2m(BoxNumberXY))
      call DefineSet( KQ1(BoxNumberXY), DIFFUSION, FOR_ALL_LAYERS, 0, diff, dummy)
      ...
      call DefineSet( KQ1(BoxNumberXY),DEFINE, 11,EXPONENTIAL_TERM, gamma,dummy)
      call DefineSet( KQ1(BoxNumberXY),DEFINE, 12,EXPONENTIAL_TERM,-gamma,dummy)
      call DefineSet( KQ1(BoxNumberXY),DEFINE, 15, CONSTANT_TERM,    dummy,dummy)

      call DefineSet( KQ1(BoxNumberXY),DEFINE, 21,ZERO_EXPONENTIAL_TERM,-alpha,dummy)
      ...
      call DefineSet( KQ1(BoxNumberXY),DEFINE, 31,ZERO_EXPONENTIAL_TERM,-alpha,dummy)
      ...
      call FixProportionCoeff(KQ1(BoxNumberXY),21,31,1.0D+00,r)
      !8:
      ...
      cQ1c = CalculateSet( KQ1(BoxNumberXY), SET_LAYER_INTEGRAL_UNTIL, &
        LAYER1, LAYER1, D1m(BoxNumberXY), ZERO)
      ...
      call LimitShift(flow,Q1c(BoxNumberXY),Q11c(BoxNumberXY),max_change_per_step)
