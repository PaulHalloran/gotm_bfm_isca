...
! INTERFACE
  ...
  function CalcCO2System(mode,salt,temp,rho,n1p,n5s,Ac,CO2,HCO3,CO3,pH, &
...
!
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    ...
    K1Ac = exp( - 2307.1266D+00/ tk+ 2.83655D+00- &
      ...
      0.08468345D+00* salt- 0.00654208D+00* s15+ log( 1.0D+00- 0.001005D+00* &
      salt))

    K2Ac = exp( - 3351.6106D+00/ tk- 9.226508D+00- &
      0.2005743D+00* dlogtk+(- 23.9722D+00/ tk- 0.10690177D+00)* sqrts+ &
      0.1130822D+00* salt- 0.00846934D+00* s15+ log( 1.0D+00- 0.001005D+00* &
      salt))
    ...
    if ( mode ==  DYNAMIC ) then
        ...
        KsAc = exp( - 4276.1D+00/ tk+ 141.328D+00- &
              ...
              114.723D+00* dlogtk+ sqrtis*(- 2698.0D+00/ tk+ sqrtis*( 1776.0D+00/ &
              tk))))+ log( 1.0D+00- 0.001005D+00* salt))
        ...
        KfAc = exp( 1590.2D+00/ tk- 12.641D+00+ &
              1.525D+00* sqrtis+ log( 1.0D+00- 0.001005D+00* salt)+ &
              log( 1.0D+00+( 0.1400D+00/ 96.062D+00)*( scl)/ KsAc))
        ...
        small_interval =( pH .gt.4.0.and.ph.lt.8.0) 

        if (small_interval) error = rtsafe( CalcHplus, pH-0.5, pH+0.5, 1.0D-6, pH)
        if ((.not.small_interval).or.error.gt.0.0) &
                          error = rtsafe( CalcHplus, 2.0D+00, 11.0D+00, 1.0D-6, pH)
     ...
     if ( mode == STATIC .or.( way==1.and.error >0) ) then
      if (Ac.gt.2.0*Dic_in) then 
        ...
        !-p/2(p) = tmp1:
        tmp1 = - 0.5D+00*( Ac-DIC_in+ 4.0D+00* K2Ac/ K1Ac*( &
          2.0D+00* DIC_in- Ac))/( 1.0D+00- 4.0D+00* K2Ac/ K1Ac)

        !q(p) = tmp2:
        tmp2 = - K2Ac* (( 2.0D+00* DIC_in- Ac))**(2.0D+00)/( &
          K1Ac*( 1.0D+00- 4.0D+00* K2Ac/ K1Ac))
        ...
        !pH:
        pH = - log( K1Ac* CO2/(2.0D+00* DIC_in &
          - Ac- 2.0D+00* CO2))/ log( 10.0D+00)
...
!          ldic=CO2*(hphp+K1AC*hp +K1Ac*K2Ac)/hphp
  ...
  lfn= lfn + KwAc/ x+ sN1p* k12p* x/ a+ 2.0D+00* &
    sN1p* k123p/ a+ sN5s/( 1.0D+00+ x/ KsiAc)- x/ c- st/( 1.0D+00+ &
    KsAc/ x/ c)- ft/( 1.0D+00+ KfAc/ x)- sN1p* x3/ a- ta
  !
  !	df = dfn/dx
  !
  ldf=ldf - KwAc/ x2+( sN1p* k12p*( &
    a- x* da))/ a2- 2.0D+00* sN1p* k123p* da/ a2- sN5s/ &
    KsiAc/ (1.0D+00+ x/ KsiAc)**(2.0D+00)- 1.0D+00/ c+ st* &
    (1.0D+00+ KsAc/ x/ c)**(- 2.0D+00)*( KsAc/ c/ x2)+ ft* &
    (1.0D+00+ KfAc/ x)**(- 2.0D+00)* KfAc/ x2- sN1p* x2*( &
    3.0D+00* a- x* da)/ a2
...
! !LOCAL
  ...
  tk100  =   (temp+273.15D+00)/ 100.0D+00
