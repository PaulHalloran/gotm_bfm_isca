...
!
  ...
  real(RLEN)  :: p_lureaN4n  ! ammonium conc. at which uptake of organic N are equal (BFM)
  ...
  namelist /PelBac_parameters/ p_version, p_q10, p_chdo, p_sd, p_sd2, p_suhR1, &
    p_sulR1, p_suR2, p_suR6, p_sulR6,p_cuR6np, p_sum, p_pu_ra, p_pu_ra_o, &
    p_pu_ea_R7, p_srs, p_qpc, p_qplc, p_qnc, p_qnlc, p_qun, p_qup, p_lN3N4n, &
    p_lureaN4n,p_lN1
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
