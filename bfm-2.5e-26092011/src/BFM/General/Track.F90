...
! !USES:
     ...
     use global_mem, only: LOGUNIT,ALLTRANSPORT
...
!            r=Source_D3_Vector(ppN3n)/(1.0D-80+N3n)
!            s=Source_D3_Vector(pptrN3n)/(1.0D-80+trN3n)
...
!----------------------------------------------------------------------------
      ...
      if ( iiTrack<=2) return;
      if ( iiSys==iiPel .and. rate <= 0.0 .and. &
      ...
      elseif ( iiSys==iiBen .and. rate > 0.0 .and. &
...
!----------------------------------------------------------------------------
               ...
               if ( PELBOTTOM(mt) < 0.0 ) then
                 dev_loss=(SINK(mt,mt)*SEC_PER_DAY +PELBOTTOM(mt)/Depth) *min(1.0,(1.0D-80+STATE(ml))/(1.0D-80+STATE(mt)))
                  ...
                  elseif (dev_loss .lt.-1.0D-04 ) then
                    ...
                    write(LOGUNIT,'(''definitive loss  rate var (/m2/day):'',G12.5)') dev_loss*Depth
                    write(LOGUNIT,'(''SINK of this state var (/m2/day):'',G12.5)') SINK(mt,mt)*Depth*SEC_PER_DAY
                    write(LOGUNIT,'(''flux to bottom  of this state var (/m2/day):'',G12.5)') PELBOTTOM(mt)
                    write(LOGUNIT,'(''value of this state var (/m3):'',G12.5)') STATE(mt)
                    write(LOGUNIT,'(''value of '',A,''  (/m3):'',G12.5)') & 
              ...
              if ( m3 .gt.0)  then
                if ( PELBOTTOM(m3) > 0.0 ) then
                  ...
                  elseif (dev_loss .lt.-1.0D-04 ) then
                    ...
                    write(LOGUNIT,'(''definitive loss  rate var (/m2/day):'',G12.5)') dev_loss
                    write(LOGUNIT,'(''SINK of this state var (/m2/day):'',G12.5)') SINK(mt,mt)*SEC_PER_DAY
                    write(LOGUNIT,'(''flux to bottom  of this state var (/m2/day):'',G12.5)') PELBOTTOM(m3)
                    write(LOGUNIT,'(''value of this state var (/m2):'',G12.5)') STATE(mt)
                    write(LOGUNIT,'(''value of '',A,''  (/m2):'',G12.5)')  &
      ...
      do mt=1,N3-ii3dTrack
        ml=nr_3d_track(mt);if (ml >0) then
          ...
          if (PELBOTTOM(mt) .ne. 0.0 .and.  m2==0)  then
              ...
              call BFM_ERROR("bencoup_track",message)
          elseif (PELBOTTOM(mt) < 0.0 .and. mf==0 )  then
      ...
      do mt=1,N2-ii2dTrack
        ml=nr_2d_track(mt);if (ml.ne.0) then
          ...
          if ( m3 .gt.0)  then
            ...
            ml3=nr_3d_track(m3)
            !only automatic  (mf==0)
            if ( PELBOTTOM(m3) > 0.0 .and.  mf ==0 ) then
...
#ifdef INCLUDE_TRACK
      use mem,only : jK3G4n,jK34K24n,jK13K3n,K3n,K24n
      use mem,only : jtrK3G4n,jtrK34K24n,jtrK13K3n,trK3n,trK24n

      implicit none

      jtrK3G4n=jK3G4n*(trK3n)/(p_small+K3n)
      jtrK13K3n=min(0.0,jK13K3n*(trK3n)/(p_small+K3n))
      jtrK34K24n=min(0.0,jK34K24n*(trK24n)/(p_small+K24n))
#endif
