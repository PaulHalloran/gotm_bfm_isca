...
!       check=0.0D+00
!       where (input > MAX_VAL_EXPFUN ) check=DBL_MAX 
!       where (input < MIN_VAL_EXPFUN ) check=DBL_MIN ;
!       where (check == 0.0D+00)
!           EXP_LIMIT=exp(check)
!       elsewhere
!           EXP_LIMIT=check
!       endwhere
        ...
        check=0
        where (input > MAX_VAL_EXPFUN ) check=1 
        where (input < MIN_VAL_EXPFUN ) check=-1 ;
        where (check == 0)
            EXP_LIMIT=exp(input)
        elsewhere
            EXP_LIMIT=0.5*(-real(check-1)*DBL_MIN +real(check+1)*DBL_MAX)
        endwhere
...
function eTq_VECTOR(temp,p_q10)
        ...
        if ( abs(p_q10-1.0D+00) < 1.0D-6) then
          eTq_VECTOR=  1.0D+00
        else
