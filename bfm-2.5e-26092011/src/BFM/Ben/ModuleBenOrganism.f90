...
! !USES:
  ...
  use global_mem
  use mem,  ONLY: iiBenOrganisms, iiY1, iiY2, iiY3, iiY4, iiY5,iiBenBacteria
...
! real(RLEN)  :: p_tH(iiBenBacteria) !bacterial type ,1.0,2.0,1.0
  ...
  namelist /BenOrganism_parameters/ p_q10, p_su, p_chu, p_clu, p_pue, p_pur, &
    p_pudil, p_sr, p_sra, p_puQ6, p_pueQ6, p_cm, p_clm, p_sd, p_sdm, p_qn, p_qp, p_Yn, &
    p_Hn
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
