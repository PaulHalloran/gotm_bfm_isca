...
!
  ...
  real(RLEN)  :: p_cuQ6np(iiBenBacteria) ! Prefferential uptake of nutrie nts of Q6
  ...
  real(RLEN)  :: p_suUQ1(iiBenBacteria)  ! Specific uptakte rate of Q1  with a high urea content(1/d)
  ...
  real(RLEN)  :: p_sd2(iiBenBacteria)  ! Specific mortality          (1/d)
  ...
  namelist /BenBac_parameters/ p_cdm, p_qpc, p_qlpc, p_qnc, p_qlnc, p_q10, &
    p_suhQ6, p_sulQ6, p_cuQ6np, p_sum, p_pue, p_suQ1,p_suUQ1, p_pur, p_srr, &
    p_sumKIn, p_sumKIp, p_sd,p_sd2, p_iK4, p_iK1, p_iQ1
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
